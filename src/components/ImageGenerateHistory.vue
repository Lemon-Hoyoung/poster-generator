...
const generateHandle = async () => {
  ...
  nextTick(() => {
    const componentsNodes = result.map((param, index) => {
      ...
      if (rootContainer === null) return undefined;
      app.mount(rootContainer);
      return rootContainer;
    }).filter(i => typeof i !== 'undefined');
    instructStore.setDomList(componentsNodes as HTMLElement[]);

    let canvasList: HTMLCanvasElement[] = [];
    const componentsImageBase64 = componentsNodes.map(async (node) => {
...
watch(() => instructStore.showFormat, (format: ShowFormatEnum) => {
  nextTick(() => {
